...
class Darknet53(tf.keras.Model):
    ...
    def call(self, input_tensor, training=False):
        ...
        #spp13 = slim.max_pool2d(x,13,stride=1,padding='SAME')
        #spp9 = slim.max_pool2d(x,9,stride=1,padding='SAME')
        #spp5 = slim.max_pool2d(x,5,stride=1,padding='SAME')
        #x=tf.concat([spp13,spp9,spp5,x],-1, name="stage6")	
        output_stage5 = x
    ...
    def _init_vars(self):
        ...
        imgs = cp.asnumpy(cp.random.randn(1, 256, 256, 3).astype(cp.float32))
...
class _ConvPoolBlock(tf.keras.Model):
    def __init__(self, filters, layer_idx, name=""):
        ...
        self.pad = layers.ZeroPadding2D(((1,0),(1,0)))#這邊可以調(1,1)(1,1)，都會變偶數((1, 104, 104, 128))
