...
class BatchGenerator(object):
    ...
    def _get(self):
		...
		#img=原本圖片尺寸轉化為416大小,boxes為原本座標轉化成416座標
...
def _encode_box(yolo, original_box, anchor_box, net_w, net_h):
    ...
    #计算物体相对于候选框的尺寸缩放值
	# 因為放大是使用自然常數倍數增加，因此縮小就是用log ，可以用此數學反推至此
...
def _find_match_anchor(box, anchor_boxes):
    ...
    max_index = find_match_box(shifted_box, anchor_boxes)#這檔適合第7個錨點
    max_anchor = anchor_boxes[max_index]#[  0,   0, 156, 198]
    scale_index = max_index // 3 #選擇網格大小
...
def _assign_box(yolo, box_index, box, label):
    ...
    #填入所计算的数值，作为标签
    yolo[grid_y, grid_x, box_index]      = 0.		#將每一個種類的list都歸0
    yolo[grid_y, grid_x, box_index, 0:4] = box		#置信度一開始都為1
    yolo[grid_y, grid_x, box_index, 4  ] = 1.       #置信度一開始都為1
    yolo[grid_y, grid_x, box_index, 5+label] = 1.	#此物體的標籤對應的list的位置，標籤[5]就是對應yolo list中的第10個位置
