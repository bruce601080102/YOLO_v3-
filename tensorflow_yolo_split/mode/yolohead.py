...
class Headnet(tf.keras.Model):
    ...
    def _init_vars(self):
        ...
        s3 = tf.constant(cp.asnumpy(cp.random.randn(1, 32, 32, 256).astype(cp.float32)))
        s4 = tf.constant(cp.asnumpy(cp.random.randn(1, 16, 16, 512).astype(cp.float32)))
        s5 = tf.constant(cp.asnumpy(cp.random.randn(1, 8, 8, 1024).astype(cp.float32)))
