...
def _create_mesh_xy(batch_size, grid_h, grid_w, n_box):#生成带序号的网格
	...
	#網格合併，因為有三個框，因此像13*13就會有座標，並且要生產出3個錨點，維度還要增加2個
...
def adjust_pred_tensor(y_pred):#将网格信息融入坐标，置信度做sigmoid。并重新组合
    ...
    pred_xy    = grid_offset + tf.sigmoid(y_pred[..., :2])  #计算该尺度矩阵上的坐标sigma(t_xy) + c_xy 論文公式錨點
...
def loss_class_tensor(object_mask, pred_box_class, true_box_class, class_scale):
    ...
    class_delta = object_mask * \
                  tf.expand_dims(tf.nn.softmax_cross_entropy_with_logits(labels=true_box_class_, logits=pred_box_class,name=None), 4) * \
                  class_scale
