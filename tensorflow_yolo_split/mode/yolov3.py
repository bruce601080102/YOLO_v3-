...
class Yolonet(tf.keras.Model):
    ...
    def _init_vars(self):
        ...
        #sample = tf.constant(cp.asnumpy(cp.random.randn(1, 256, 256, 3).astype(cp.float32)))
...
def preprocess_input(image, net_size):
    ...
    preprocess_img = cv2.resize(image/255., (net_size, net_size),interpolation = cv2.INTER_AREA)
